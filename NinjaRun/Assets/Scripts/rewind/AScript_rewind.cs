...
public class AScript_rewind : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.Alpha2) || Input.GetKeyDown(KeyCode.Alpha1) || Input.GetKeyDown(KeyCode.Alpha4)){
                canBePressed = false;
                //objectText.text = (int.Parse(objectText.text)-1).ToString();
            }
            else if(Input.GetKeyDown(keyToPress)){
                gameObject.SetActive(false);
                Debug.Log(Slow_rewind.slow);
                if (Slow_rewind.slow) {
                    objectText.text = (int.Parse(objectText.text)+2f).ToString();
                } 
                else if (Fast_rewind.fast) {
    ...
    private void createRandomObstacle(){
        ...
        cube.AddComponent<ObstacleCollision_rewind>();

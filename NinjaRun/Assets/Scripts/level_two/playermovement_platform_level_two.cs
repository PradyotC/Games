...
public class playermovement_platform_level_two : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      // End game condition
      if (((int)transform.position.x >= 370 || (int)transform.position.y < -2) && !isGameEnded ){
            // changes
            MetricsVariables.laneSwitches = originalnumberOfM - numberOfM;
            if ((int)transform.position.x >= 370)
            {
                MetricsVariables.finished = "Yes";
            }
            menu.SetActive(true);
            GameObject.Find("ResumeGame").SetActive(false);

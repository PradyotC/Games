...
public class SScript_level2 : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.Alpha4) || Input.GetKeyDown(KeyCode.Alpha3) || Input.GetKeyDown(KeyCode.Alpha1)){
                canBePressed = false;
                objectText.text = (int.Parse(objectText.text)-10).ToString();
                 ...
                 objectTextEnemy.text = (int.Parse(objectTextEnemy.text)-5).ToString();
                PlayerFire();

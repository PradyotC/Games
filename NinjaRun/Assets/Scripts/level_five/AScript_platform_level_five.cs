...
public class AScript_platform_level_five : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        ...
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.W) || Input.GetKeyDown(KeyCode.E) ){
                ...
                if (playermovement_platform_level_five.isSlow) {
                    objectText.text = (int.Parse(objectText.text)+2f).ToString();
                } 
                else if (playermovement_platform_level_five.isFast) {
            ...
            else if (playermovement_platform_level_five.isDashing){
                objectText.text = (int.Parse(objectText.text)+5f).ToString();
                canBePressed = false;
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.name == "segment5" && !playermovement_platform_level_five.isDashing && !playermovement_platform_level_five.isSwitchGravity)
        {
            //Fire();

...
public class playermovement_platform_level_five : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      // End game condition
      if (((int)transform.position.x == 335 || (int)transform.position.y < -2) && !isGameEnded ){
        // changes
            MetricsVariables.laneSwitches = originalnumberOfM - numberOfM;
         if ((int)transform.position.x >= 335)
            {

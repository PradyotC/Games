...
public class playermovement_level_Siddhant : MonoBehaviour
{
    ...
    public List<string> collectibles = new List<string>(); // List to maintain only one collectible in storage
    ...
    void Start()
    {
        collectibles = new List<string>(); // reintialize at every restart
    ...
    void Update()
    {
      ...
      if(Gravity_level_Siddhant.reverseGravity){
      ...
      if ((int)transform.position.x == 300){
        ...
        if (myCharacterController.isGrounded && jumpUp && Reverse_Functionality_level_Siddhant.reverseFunctionality == false && moveVector.z == 0) {
        ...
        else if (jumpUp && transform.position.y<4 && (moveVector.y > 0f || moveVector.y < 0f) && Reverse_Functionality_level_Siddhant.reverseFunctionality == false && double_jump_active==1) {
          Debug.Log("Double Jump");
          moveVector.y = jumpSpeed+1.25f;
          double_jump_active = 0;
        }
        if (Input.GetKeyDown(KeyCode.LeftShift ) && Reverse_Functionality_level_Siddhant.reverseFunctionality == false) {
          ducktime = 180;
    ...
    // set collectible to be used later
    void setCollectible(string collectible)
    {
        ...
        Debug.Log("[" + string.Join(", ", collectibles) + "]");
    }

    // get collectible when triggered by key
    void getCollectible()
    {
        Debug.Log("[" + string.Join(", ", collectibles) + "]");
        if (collectibles.Count == 1)
        {
            string collectible = collectibles[0];
            collectibles.RemoveAt(0);
            switch (collectible)
            {
                case "fast":
    ...
    private IEnumerator Dash(float new_speed, float original_speed, float time) {
      ...
      tr.emitting = false;
      Dash_level_Siddhant.dash = false;
    ...
    private IEnumerator ReverseFunctions(float original_speed, float time) {
      ...
      Reverse_Functionality_level_Siddhant.reverseFunctionality = true;
      speed = original_speed;
      ...
      // tr.emitting = false;
      Reverse_Functionality_level_Siddhant.reverseFunctionality = false;
      speed = original_speed;
      // speed = original_speed;
      // audioData.pitch = 1;
    }
...
public IEnumerator ReverseSpeed(float new_speed, float original_speed, float time) {
      ...
      audioData.pitch = new_speed/original_speed;
      yield return new WaitForSeconds(time);
      Reverse_level_Siddhant.reverse = false;
      speed = original_speed;
      audioData.pitch = 1f;
    }
...
public IEnumerator ReverseGravity(float time) {
      ...
      Gravity_level_Siddhant.reverseGravity = false;
    }

...
public class AScript_platform_level_one : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        ...
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.W) || Input.GetKeyDown(KeyCode.E) ){
                ...
                if (playermovement_platform_level_one.isSlow) {
                    objectText.text = (int.Parse(objectText.text)+2f).ToString();
                } 
                else if (playermovement_platform_level_one.isFast) {
            ...
            else if (playermovement_platform_level_one.isDashing){
                objectText.text = (int.Parse(objectText.text)+5f).ToString();
                canBePressed = false;
            }
        }
    ...
    {
        //Player Rhythmm collector
        if (other.name == "Ch24_nonPBR@Running")
        // if (other.name == "segment4")
        {
            canBePressed = true;
        }
        if(other.name == "segment5" && !playermovement_platform_level_one.isDashing && !playermovement_platform_level_one.isSwitchGravity)
        {
            //Fire();
            GenerateNodes_platform_level_one.cnt+=1;
            if (GenerateNodes_platform_level_one.cnt == 5){
                createRandomObstacle();
                GenerateNodes_platform_level_one.cnt=0;
            }
            //objectText.text = (int.Parse(objectText.text)-1).ToString();

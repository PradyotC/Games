...
public class playermovement_level_ayan : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      if(Gravity_level_ayan.reverseGravity){
      ...
      if (Slow_level_ayan.slow) {
        ...
        // Slow_level_ayan.slow = false;
      }

      if (Fast_level_ayan.fast) {
      ...
      if (Dash_level_ayan.dash) {
        setCollectible("dash");
        // StartCoroutine(Dash(dashed_speed_of_player,original_speed_of_player,dashingTime));
        Dash_level_ayan.dash = false;
      }

      if (Reverse_Functionality_level_ayan.reverseFunctionality) {
      ...
      if ((int)transform.position.x == 300){
        ...
        if (myCharacterController.isGrounded && jumpUp && Reverse_Functionality_level_ayan.reverseFunctionality == false && moveVector.z == 0) {
        ...
        else if (jumpUp && transform.position.y<4 && (moveVector.y > 0f || moveVector.y < 0f) && Reverse_Functionality_level_ayan.reverseFunctionality == false && double_jump_active==1) {
          Debug.Log("Double Jump");
          moveVector.y = jumpSpeed+1.25f;
          double_jump_active = 0;
        }
        if (Input.GetKeyDown(KeyCode.LeftShift ) && Reverse_Functionality_level_ayan.reverseFunctionality == false) {
          ducktime = 180;
    ...
    void setCollectible(string collectible)
    {
        if (collectibles.Count==0)
        {
            ...
            // Debug.Log("["+string.Join(", ", collectible)+"]");
        }
        else
        {
    ...
    private IEnumerator ReverseFunctions(float original_speed, float time) {
      ...
      Reverse_Functionality_level_ayan.reverseFunctionality = true;
      speed = original_speed;
      ...
      // tr.emitting = false;
      Reverse_Functionality_level_ayan.reverseFunctionality = false;
      speed = original_speed;
      // speed = original_speed;
      // audioData.pitch = 1;
    }
...
public IEnumerator ReverseGravity(float time) {
      ...
      Gravity_level_ayan.reverseGravity = false;
    }

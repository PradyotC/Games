...
public class playermovement_platform_level_three : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      // End game condition
      if (((int)transform.position.x == 350 || (int)transform.position.y < -2) && !isGameEnded ){
            // changes
            MetricsVariables.laneSwitches = originalnumberOfM - numberOfM;
            if ((int)transform.position.x >= 350)
            {

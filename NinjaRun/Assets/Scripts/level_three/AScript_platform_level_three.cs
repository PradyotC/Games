...
public class AScript_platform_level_three : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        ...
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.W) || Input.GetKeyDown(KeyCode.E) ){
                ...
                if (playermovement_platform_level_three.isSlow) {
                    objectText.text = (int.Parse(objectText.text)+2f).ToString();
                } 
                else if (playermovement_platform_level_three.isFast) {
            ...
            else if (playermovement_platform_level_three.isDashing){
                objectText.text = (int.Parse(objectText.text)+5f).ToString();
                canBePressed = false;
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.name == "segment5" && !playermovement_platform_level_three.isDashing && !playermovement_platform_level_three.isSwitchGravity)
        {
            //Fire();

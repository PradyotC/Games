...
public class AScript_platform_level_four : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        ...
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.W) || Input.GetKeyDown(KeyCode.E) ){
                ...
                if (playermovement_platform_level_four.isSlow) {
                    objectText.text = (int.Parse(objectText.text)+2f).ToString();
                } 
                else if (playermovement_platform_level_four.isFast) {
            ...
            else if (playermovement_platform_level_four.isDashing){
                objectText.text = (int.Parse(objectText.text)+5f).ToString();
                canBePressed = false;
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.name == "segment5"&& !playermovement_platform_level_four.isDashing && !playermovement_platform_level_four.isSwitchGravity)
        {
            //Fire();
            GenerateNodes_platform_level_four.cnt+=1;
            if (GenerateNodes_platform_level_four.cnt == 3){
                createRandomObstacle();
                GenerateNodes_platform_level_four.cnt = 0;
            }

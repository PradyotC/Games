...
public class SScript_platform_level_seven : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        ...
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.Q) || Input.GetKeyDown(KeyCode.E)){
                ...
                if (playermovement_platform_level_seven.isSlow) {
                    objectText.text = (int.Parse(objectText.text)+2f).ToString();
                } 
                else if (playermovement_platform_level_seven.isFast) {
            ...
            else if (playermovement_platform_level_seven.isDashing){
                objectText.text = (int.Parse(objectText.text)+5f).ToString();
                canBePressed = false;
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.name == "segment5" && !playermovement_platform_level_seven.isDashing && !playermovement_platform_level_seven.isSwitchGravity)
        {
            //Fire();

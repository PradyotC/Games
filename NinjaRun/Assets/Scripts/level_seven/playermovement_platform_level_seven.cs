...
public class playermovement_platform_level_seven : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      // End game condition
      if (((int)transform.position.x >= 500 || (int)transform.position.y < -2) && !isGameEnded ){
            // changes
            MetricsVariables.laneSwitches = originalnumberOfM - numberOfM;
         if ((int)transform.position.x >= 500)
            {
    ...
    private IEnumerator ReverseFunctions(float original_speed, float time)
    {
        ...
        int prevPosition = (int)player.transform.position.x;
        ...
        // Double Jump condition
        else if (LeftShift && transform.position.y < 4 && (moveVector.y > 0f || moveVector.y < 0f) && Reverse_Functionality_level1.reverseFunctionality == true && double_jump_active == 1)
        {
            Debug.Log("Double Jump");
            moveVector.y = jumpSpeed + 5.25f;
            double_jump_active = 0;
        }
        if (jumpUp)
        {
        ...
        //cube.AddComponent<TextMeshPro>();
        //TextMeshPro reversetext = cube.GetComponent<TextMeshPro>();
        //reversetext.transform.position = new Vector3(prevPosition + time * speed + 8f, 6.45f, -3.81f);
        //reversetext.text = "Back Normal";
        //reversetext.fontSize = 10f;

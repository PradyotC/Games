...
public class DScript_level_dash : MonoBehaviour
{
    GameObject Health;
    ...
    void Update()
    {
        ...
        if(canBePressed)
        {
            if(Input.GetKeyDown(KeyCode.Alpha2) || Input.GetKeyDown(KeyCode.Alpha3) || Input.GetKeyDown(KeyCode.Alpha4)){
                ...
                if (playermovement_level_slow.isSlow) {
                    objectText.text = (int.Parse(objectText.text)+2f).ToString();
                } 
                else if (playermovement_level_fast.isFast) {
            ...
            else if (playermovement_level_dash.isDashing){
                objectText.text = (int.Parse(objectText.text)+5f).ToString();
                canBePressed = false;
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(other.name == "segment5" && !playermovement_level_dash.isDashing && !playermovement_level_gravity.isSwitchGravity)
        {
    ...
    private void createRandomObstacle(){
        ...
        cube.AddComponent<ObstacleCollision_level_dash>();
        ...
        else if (obstacleType == 1)
        {
            ...
            cube.AddComponent<ObstacleMovementUp_level_dash>();
        }
        else if (obstacleType == 2)
        {
            cube.transform.position = new Vector3(playerPosition + 10f, target.transform.position.y + 0.25f, target.transform.position.z);
            cube.AddComponent<ObstacleMovementRight_level_dash>();
        }
        else if (obstacleType == 3)
        {
            cube.transform.position = new Vector3(playerPosition + 10f, target.transform.position.y + 2f, target.transform.position.z);
            cube.AddComponent<ObstacleMovementSinusoidal_level_dash>();
        }

...
public class playermovement_level_dash : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      if ((int)transform.position.x == 25 && dash_tutorial)
      {
        ...
        dash_cube=GameObject.CreatePrimitive(PrimitiveType.Cube);
          ...
          TextDisplay.text = "Dash enables Ninja to pass through obstacles without\n losing points and also get points on all\n rhythm nodes, Press Enter to continue";
          TextDisplay.fontSize = 6;
          TextDisplay.color = new Color32(255, 128, 0, 255);
          RectTransform placement = dash_cube.GetComponent<RectTransform>();
          dash_cube.transform.position = new Vector3(transform.position.x+10, transform.position.y+3, transform.position.z-2);
      ...
      if ((int)transform.position.x == 40 && dash_tutorial_launch)
      {
        ...
        dash_cube_launch=GameObject.CreatePrimitive(PrimitiveType.Cube);
          ...
          TextDisplay.text = "Press N to use the Dash";
      ...
      /// Dashing -> increases the speed
       ...
       if (Input.GetKeyDown(KeyCode.N) && collectible_id==1 && Dash_level_dash.dash){
          StartCoroutine(Dash(dashed_speed_of_player,original_speed_of_player,dashingTime));
          Dash_level_dash.dash=false;
          collectible_id=0;
        }
